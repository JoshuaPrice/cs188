...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        for i in range(len(parents)):
            inputs.append(self.node_values[parents[i]][0])
    ...
    def add(self, node):
        ...
        outp = node.forward(self.get_inputs(node))
        gradient = np.zeros_like(outp)
        self.node_values[node] = [outp, gradient]
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        backOrder = self.get_nodes()[::-1] # reverse order of nodes excluding loss node
        for node in backOrder:
            if node == loss_node:
                node_grad = np.array([1.0])
                self.node_values[node][1] = node_grad
            else:
                node_grad = self.node_values[node][1]
            node_parents = node.get_parents()
            parents_grads = node.backward(self.get_inputs(node), node_grad)
            for p in range(len(node_parents)):
                self.node_values[node_parents[p]][1] = self.node_values[node_parents[p]][1] +  parents_grads[p]
    ...
    def step(self, step_size):
        ...
        """
        "*** YOUR CODE HERE ***"
        # decrease the data's value by the node's gradient times the step_size parameter
        for each in self.variables: 
            each.data -= self.node_values[each][1]*step_size
...
class Add(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [gradient for i in range(len(inputs))]
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        delfdelA = np.dot(gradient, np.transpose(inputs[1]))
        delfdelB = np.dot(np.transpose(inputs[0]), gradient)
        return [delfdelA, delfdelB]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        a = inputs[0]
        b = inputs[1]
        return [gradient*(a-b)/np.size(a), gradient*(b-a)/np.size(a)]
