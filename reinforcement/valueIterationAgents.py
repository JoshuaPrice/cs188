...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in xrange(self.iterations):
          ...
          if not self.mdp.isTerminal(prioritizedState):
            act = self.getAction(prioritizedState)
            maxQVal = self.getQValue(prioritizedState, act)
            print "updating state ", prioritizedState, "with value", maxQVal
            self.values[prioritizedState] = maxQVal

            for pred in predecessors[prioritizedState]:
              if not self.mdp.isTerminal(pred):
                act = self.getAction(pred)
                maxQVal = self.getQValue(pred, act)
