...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        queuedStatesAndVals = util.PriorityQueue()
        ...
        for st in states:
          if not self.mdp.isTerminal(st):
            ...
            diff = abs(maxQVal - self.values[st])
            print "Adding to queue state", st, " with priority ", -1*diff
            queuedStatesAndVals.update((st, maxQVal), -1*diff)

        for i in xrange(self.iterations): # need this?
          if queuedStatesAndVals.isEmpty():
            print "Queue is empty. Terminating runValueIteration."
            return None
          prioritizedStateAndVal = queuedStatesAndVals.pop()
          prioritizedState = prioritizedStateAndVal[0]
          prioritizedVal = prioritizedStateAndVal[1]
          ...
          for pred in predecessors[prioritizedState]:
            if not self.mdp.isTerminal(pred):
              ...
              if diff > self.theta: 
                ...
                queuedStatesAndVals.update((pred, maxQVal), -1*diff)
