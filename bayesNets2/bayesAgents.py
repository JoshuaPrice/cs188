...
def constructBayesNet(gameState):
    ...
    # full set of observation variables
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(obsVar) # opulate `obsVars`
            edges.append((GHOST_HOUSE_VAR, obsVar)) # populate `edges` with every edge in the Bayes Net (a tuple `(from, to)`)
            edges.append((FOOD_HOUSE_VAR, obsVar))
            variableDomainsDict[obsVar] = [BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL] #set each `variableDomainsDict[var] = values`
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                red, blue, none = 0, 0, 0 
                #get the House value corresponding to positioon obsPos[0], obsPos[1]
                w = gameState.data.layout.width
                h = gameState.data.layout.height
                houseVal = None
                if obsPos[1] < h/2.0 and obsPos[0] < w/2.0:
                	houseVal = BOTTOM_LEFT_VAL
                elif obsPos[1] < h/2.0 and obsPos[0] > w/2.0:
                	houseVal = BOTTOM_RIGHT_VAL
                elif obsPos[1] > h/2.0 and obsPos[0] < w/2.0:
                	houseVal = TOP_LEFT_VAL
                elif obsPos[1] > h/2.0 and obsPos[0] > w/2.0:
                	houseVal = TOP_RIGHT_VAL
                if assignment[FOOD_HOUSE_VAR] == houseVal:
                    red = PROB_FOOD_RED
                    blue = 1 - PROB_FOOD_RED
                    none = 0
                elif assignment[GHOST_HOUSE_VAR] == houseVal:
                    red = PROB_GHOST_RED
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    probabilistic_query = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        "*** YOUR CODE HERE ***"
        probabilistic_query = inference.inferenceByVariableElimination(self.bayesNet, [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR],
         evidence, eliminationOrder)

        allPossibleAssignments = probabilistic_query.getAllPossibleAssignmentDicts()

        # make probability-storing variables
        foodTRghostTLprob = 0
        foodTLghostTRprob = 0

        for assignment in allPossibleAssignments:
            if assignment['foodHouse'] == 'topRight' and assignment['ghostHouse'] == 'topLeft':
                foodTRghostTLprob = probabilistic_query.getProbability(assignment)
            if assignment['foodHouse'] == 'topLeft' and assignment['ghostHouse'] == 'topRight':
                foodTLghostTRprob = probabilistic_query.getProbability(assignment)

        leftExpectedValue = foodTLghostTRprob * WON_GAME_REWARD + foodTRghostTLprob * GHOST_COLLISION_REWARD
        rightExpectedValue = foodTLghostTRprob * GHOST_COLLISION_REWARD + foodTRghostTLprob * WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for pair in explorationProbsAndOutcomes:
            explorationProb = pair[0]
            explorationOutcome = pair[1]
            expectedValOfActionGivenEvidence = max(self.computeEnterValues(explorationOutcome, enterEliminationOrder))
            expectedValue += explorationProb * expectedValOfActionGivenEvidence
