...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # grab all factors where we know the evidence variables (to reduce the size of the tables)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        f = currentFactorsList

        for currentVariable in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(f, currentVariable)
            f = []
           # currentFactorsList.append(joinedFactor)
            #partialJoint = joinFactors(currentFactorsList)
            # incrementallyMarginalizedJoint = partialJoint
            if len(joinedFactor.unconditionedVariables()) > 1: 
            	incrementallyMarginalizedJoint = eliminate(joinedFactor, currentVariable)
            	f.append(incrementallyMarginalizedJoint)
            f += currentFactorsList
            # do I update currentFactorsList variable or whole bayesNet (and if whole BayesNet, what method?)
        joint = joinFactors(f)
        queryConditionedOnEvidence = normalize(joint)
        return queryConditionedOnEvidence
